...
// Animated Counters and Interactive Map for Good At Section
function animateCounter(id, end, duration, decimals = 0, suffix = '') {
  ...
  function step(timestamp) {
    ...
    value = decimals ? value.toFixed(decimals) : Math.floor(value);
    // For satisfactionCounter, keep the star icon
    if (id === 'satisfactionCounter') {
      el.innerHTML = value + ' <span class="good-at-star" style="color:#FFD700;font-size:1.3em;vertical-align:middle;">★</span>';
    } else {
    ...
    if (progress < 1) {
      ...
      if (id === 'satisfactionCounter') {
        el.innerHTML = end + ' <span class="good-at-star" style="color:#FFD700;font-size:1.3em;vertical-align:middle;">★</span>';
      } else {
...
function goodAtSectionInit() {
  const section = document.getElementById('good-at-section');
  if (!section) return;
  let animated = false;
  function onScroll() {
    const rect = section.getBoundingClientRect();
    if (!animated && rect.top < window.innerHeight - 100) {
      animateCounter('ordersCounter', 350, 1200, 0, '+');
      animateCounter('satisfactionCounter', 4.7, 1200, 1, '');
      animated = true;
  ...
  // Interactive Map Tooltip (hover and tap for mobile)
  const map = document.getElementById('goodAtMap');
  const tooltip = document.getElementById('goodAtMapTooltip');
  if (map && tooltip) {
    function showTooltip(e, country) {
      ...
      if (e.touches && e.touches.length) {
        x = e.touches[0].clientX - mapRect.left;
        y = e.touches[0].clientY - mapRect.top;
      } else {
    ...
    map.addEventListener('mousemove', function(e) {
      ...
      if (target.classList.contains('good-at-pin')) {
    ...
    map.addEventListener('touchstart', function(e) {
      ...
      if (touch.classList.contains('good-at-pin')) {
        const country = touch.getAttribute('data-country');
        showTooltip(e, country);
      }
    });
...
document.addEventListener('DOMContentLoaded', goodAtSectionInit); 